...
func NewFUBARAPI(basePath, hyPhyPath string, scheduler SchedulerInterface, datasetTracker DatasetTracker, jobTracker JobTracker) *FUBARAPI {
	return &FUBARAPI{
		HyPhyBaseAPI: NewHyPhyBaseAPI(basePath, hyPhyPath, scheduler, datasetTracker, jobTracker),
	}
}

// formatFUBARJobResults formats the raw JSON results for FUBAR jobs
func (api *FUBARAPI) formatFUBARJobResults(jobId string, rawResults json.RawMessage) (map[string]interface{}, error) {
	...
	// Return the FubarResult directly as a map to match the API spec
	// The spec expects: {"job_id": "...", "result": {...}}
	resultMap := map[string]interface{}{
		"job_id": jobId,
		"result": fubarResult.Result,
	}
...
func (api *FUBARAPI) GetFUBARJob(c *gin.Context) {
	...
	formattedResult, err := api.formatFUBARJobResults(jobId, rawResults)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// GetFUBARJobById retrieves the status and results of a FUBAR job by job ID from query parameter
func (api *FUBARAPI) GetFUBARJobById(c *gin.Context) {
	...
	result, err := api.HyPhyBaseAPI.HandleGetJobById(jobId, MethodFUBAR)
	if err != nil {
