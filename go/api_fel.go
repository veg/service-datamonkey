...
func NewFELAPI(basePath, hyPhyPath string, scheduler SchedulerInterface, datasetTracker DatasetTracker, jobTracker JobTracker) *FELAPI {
	return &FELAPI{
		HyPhyBaseAPI: NewHyPhyBaseAPI(basePath, hyPhyPath, scheduler, datasetTracker, jobTracker),
	}
}

// formatFELJobResults formats the raw JSON results for FEL jobs
func (api *FELAPI) formatFELJobResults(jobId string, rawResults json.RawMessage) (map[string]interface{}, error) {
	...
	// Return the FelResult directly as a map to match the API spec
	// The spec expects: {"job_id": "...", "result": {...}}
	resultMap := map[string]interface{}{
		"job_id": jobId,
		"result": felResult.Result,
	}
...
func (api *FELAPI) GetFELJob(c *gin.Context) {
	...
	formattedResult, err := api.formatFELJobResults(jobId, rawResults)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (api *FELAPI) StartFELJob(c *gin.Context) {
	// Get or create user session - this will automatically add X-Session-Token header if new session created
	var subject string
	if api.SessionService != nil {
		var err error
...
// GetFELJobById retrieves the status and results of a FEL job by job ID from query parameter
func (api *FELAPI) GetFELJobById(c *gin.Context) {
	...
	result, err := api.HyPhyBaseAPI.HandleGetJobById(jobId, MethodFEL)
	if err != nil {
