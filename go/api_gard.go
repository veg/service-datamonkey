...
func NewGARDAPI(basePath, hyPhyPath string, scheduler SchedulerInterface, datasetTracker DatasetTracker, jobTracker JobTracker) *GARDAPI {
	return &GARDAPI{
		HyPhyBaseAPI: NewHyPhyBaseAPI(basePath, hyPhyPath, scheduler, datasetTracker, jobTracker),
	}
}

// formatGARDJobResults formats the raw JSON results for GARD jobs
func (api *GARDAPI) formatGARDJobResults(jobId string, rawResults json.RawMessage) (map[string]interface{}, error) {
...
// GetGARDJobById retrieves the status and results of a GARD job by job ID from query parameter
func (api *GARDAPI) GetGARDJobById(c *gin.Context) {
	...
	result, err := api.HyPhyBaseAPI.HandleGetJobById(jobId, MethodGARD)
	if err != nil {
