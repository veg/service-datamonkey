...
func (api *FADEAPI) formatFADEJobResults(jobId string, rawResults json.RawMessage) map[string]interface{} {
	...
	var fadeResult FadeResult
	if err := json.Unmarshal([]byte(wrappedJSON), &fadeResult); err != nil {
...
// GetFadeJob retrieves the status and results of a FADE job
func (api *FADEAPI) GetFadeResults(c *gin.Context) {
	...
	result, err := api.HandleGetJob(c, adapted, MethodFADE)
	if err != nil {
	...
	// Parse the raw JSON results into FadeResult
...
func (api *FADEAPI) StartFadeJob(c *gin.Context) {
	...
	result, err := api.HandleStartJob(c, adapted, MethodFADE)
	if err != nil {
...
// GetFadeResultsById retrieves the status and results of a FADE job by job ID from query parameter
func (api *FADEAPI) GetFadeResultsById(c *gin.Context) {
	// Require valid token for accessing results
	if api.SessionService != nil {
		_, err := api.SessionService.GetSubject(c)
		if err != nil {
	...
	result, err := api.HyPhyBaseAPI.HandleGetJobById(jobId, MethodFADE)
	if err != nil {
