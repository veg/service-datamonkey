...
// formatMULTIHITJobResults formats the raw JSON results for MULTI-HIT jobs
func (api *MULTIHITAPI) formatMULTIHITJobResults(jobId string, rawResults json.RawMessage) (map[string]interface{}, error) {
...
func (api *MULTIHITAPI) GetMULTIHITJob(c *gin.Context) {
	...
	formattedResult, err := api.formatMULTIHITJobResults(jobId, rawResults)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// GetMULTIHITJobById retrieves the status and results of a MULTI-HIT job by job ID from query parameter
func (api *MULTIHITAPI) GetMULTIHITJobById(c *gin.Context) {
	...
	result, err := api.HyPhyBaseAPI.HandleGetJobById(jobId, MethodMULTIHIT)
	if err != nil {
